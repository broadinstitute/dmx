...
- Wrap altair charts in `mo.ui.altair_chart(...)` or molab will not render
  them. A bare `chart` expression emits the raw vega-lite spec as
  `application/vnd.vegalite.v6+json`, which expects the viewer to ship a
  vega-lite renderer. The marimo live editor does; molab's static viewer
  does not bundle vega-lite v6, so the cell paints blank even though the
  spec is fully present in the snapshot. Wrapping in
  `mo.ui.altair_chart(chart)` flips the output to `text/html` containing a
  `<marimo-vega>` custom element, and the marimo runtime that molab *does*
